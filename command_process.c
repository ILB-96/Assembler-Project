...
int command_data_process(plw *prv_DC, char *line, int line_number) {
  ...
  while (s_line[++i] != NULL &&
         !error) /*start the loop from i=1 because the the numbers in the data
                    start from 1*/
  {
    /*in case it's number*/
    if (is_only_digits(s_line[i]) == 1 && i % 2 != 0) {
      add_num_to_list(prv_DC, A, atoi(s_line[i]));
    } else if ((i % 2 == 0 && strcmp(s_line[i], ",") != 0) ||
               (i % 2 != 0 && is_only_digits(s_line[i]) == 0)) {
      error = TRUE;
...
int is_operator(char *op) {
  int result = -1;
  char *operators_array[] = {"mov", "cmp", "add", "sub", "lea", "clr",
...
int command_code_process(
    plw *prv_IC, char *line,
    int line_number) /*TODO need to get the number line for g_errors*/
{
  ...
  char **s_line = split_line(command);
  for (i = 0; i < SUM_OPERATIONS; i++) {
    if (operator= is_operator(s_line[0]) >= 0)
      error = TRUE;
  }
  ...
  case 0: /*mov*/
    error =
        add_parameters(prv_IC, s_line + 1, o_mov, f_mov, two_op, line_number);
    break;
  ...
  case 2: /*add*/
    error =
        add_parameters(prv_IC, s_line + 1, o_add, f_add, two_op, line_number);
    break;
  case 3: /*sub*/
    error =
        add_parameters(prv_IC, s_line + 1, o_sub, f_sub, two_op, line_number);
    break;
  case 4: /*lea*/
...
int add_parameters(plw *prv, char **comm, opcode opcod, Funct funct,
                   Valid_operator op, int line_number) {
  unsigned int error = FALSE;
  ...
  } else if (size == 1) /*case one operator*/
  {
    ...
    error |= !add_std_word(prv, are, funct, source_r, source_sort, target_r,
                           target_sort);
...
int add_word_by_source(plw *prv, char *comm, sortType source_sort,
                       Valid_operator op, int line_number) {
  unsigned int error = FALSE;
  switch (source_sort) {
  case immediate:
  ...
  case register_direct:
    if (op == min_two_op &&
        source_sort == register_direct) /*why the 'case' don't do it's work???*/
    {
      error = TRUE;
      fprintf(stderr, "Error at line %d: '%s' is an illegal operation\n",
              line_number, comm);
...
int add_word_by_target(plw *prv, char *comm, sortType target_sort,
                       Valid_operator op, int line_number) {
  unsigned int error = FALSE;
  switch (target_sort) {
  case immediate:
    if ((op == full_two_op || op == full_target_op) &&
        is_only_digits(comm + 1)) {
...
int set_sort_and_register(char *operator, registers * r, sortType * sort,
                          ARE * are) {
  int result = 1;
  int temp;
  *are = A;
  /*case it's register*/
  if (operator[0] == 'r' && is_only_digits(operator+ 1) &&
      (temp = atoi(operator+ 1) >= r0) && temp <= r15) {
    *r = temp;
  ...
  /*case it's variable*/
    ...
    if (isdigit(operator[0]) || is_operator(operator) >= 0)
      ...
      while (operator[i] != '\0') {
        if (parenthesis == 1 &&
            operator[i] == 'r' &&(reg = atoi(operator+ i + 1)) <= r15 &&
            reg >= r0) {
