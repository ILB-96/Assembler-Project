...
void assembler(char *file_name) {
    ...
    fflush(stdout);
    if (!pre_assembler(expanded_file_name, file_name)) { /*go to pre-assembler.c for more info*/
    ...
    if (first_pass(expanded_file_handler)) { /*Go to first-pass.c for more info*/
        fclose(expanded_file_handler);
        free(expanded_file_name);
        free_symbols_str(symbol_table);
        free(symbol_table);
        return;
    }
    printf("Assembly Second Pass at work...\n");
    if (!second_pass(expanded_file_handler)) { /*Go to second-pass.c for more info*/
        fclose(expanded_file_handler);
        free(expanded_file_name);
        free_symbols_str(symbol_table);
        free(symbol_table);
